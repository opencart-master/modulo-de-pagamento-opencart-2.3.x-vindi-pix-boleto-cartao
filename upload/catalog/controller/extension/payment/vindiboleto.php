...
class ControllerExtensionPaymentVindiboleto extends Controller {
	public function index() {
		...
		return $this->load->view('extension/payment/payment_vindiboleto', $data);
	}

	public function confirm() {
	    $json = array(); 
		if ($this->session->data['payment_method']['code'] == 'payment_vindiboleto') {
        include_once('payment_vindiboletocode.php');
	    }
	...
	if ($this->config->get('payment_vindiboleto_type') == 0) {
	...
	public function callback() {
	    ...
	    if ($this->request->server['REQUEST_METHOD'] == 'POST') {
			...
			if ($order_info && $this->request->post['token_transaction'] && $this->request->post['transaction']['transaction_id'] && $order_info['payment_code'] == 'payment_vindiboleto') {
