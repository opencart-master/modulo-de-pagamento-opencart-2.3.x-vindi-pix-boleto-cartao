...
class ControllerExtensionPaymentYapayb extends Controller {
	...
	if ($this->config->get('yapayb_type') == 0) {
	...
	public function callback() {
	    ...
	    if ($this->request->server['REQUEST_METHOD'] == 'POST') {
			...
			if ($order_info && $this->request->post['token_transaction'] && $this->request->post['transaction']['transaction_id'] && $order_info['payment_code'] == 'yapayb') {
				...
				$order_status_id = $this->config->get('yapayb_order_status_id');
				...
				switch($this->request->post['transaction']['status_id']) {
					case '4':
						...
						break;
					case '6':
						$order_status_id = $this->config->get('yapayb_order_status_id2');
						break;
					case '7':
						$order_status_id = $this->config->get('yapayb_order_status_id1');
						break;
					case '24':
						$order_status_id = $this->config->get('yapayb_order_status_id3');
						break;
					case '87':
						$order_status_id = $this->config->get('yapayb_order_status_id');
						break;
