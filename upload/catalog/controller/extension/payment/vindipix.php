...
class ControllerExtensionPaymentVindipix extends Controller {
	public function index() {
		...
		return $this->load->view('extension/payment/payment_vindipix', $data);
	}

	public function confirm() {
	    $json = array(); 
		if ($this->session->data['payment_method']['code'] == 'payment_vindipix') {
        include_once('payment_vindipixcode.php');
	    }
	...
	public function callback() {
	    ...
	    if ($this->request->server['REQUEST_METHOD'] == 'POST') {
			...
			if ($order_info && $this->request->post['token_transaction'] && $this->request->post['transaction']['transaction_id'] && $order_info['payment_code'] == 'payment_vindipix') {
