...
class ControllerExtensionPaymentVindipix extends Controller {
	...
	public function confirm() {
        ...
        $this->vindi = new VindiApi($this->registry);
			...
			if (!empty($order_info['shipping_custom_field'][$this->config->get('vindipix_complement')])) {
			$complement2 = $order_info['shipping_custom_field'][$this->config->get('vindipix_complement')];  
			} else {
			$complement2 = '';  	
			}
			$val["token_account"]  = $this->config->get('vindipix_token');
            ...
            $val["customer"]["addresses"][0]["number"] = $order_info['shipping_custom_field'][$this->config->get('vindipix_number')];
			...
			$val["customer"]["company_name"] = $campos[$this->config->get('vindipix_raz')];
			$val["customer"]["trade_name"] =  $campos[$this->config->get('vindipix_raz')];
			} 
			if (!empty($campos[$this->config->get('vindipix_doc')])) {
			$doc = preg_replace("/[^0-9]/", "", $campos[$this->config->get('vindipix_doc')]);
			...
			if($this->config->get('vindipix_days') == '') {
            $num = 0;
			} else {
			$num = $this->config->get('vindipix_days');	
			}
            $hoje = date('d-m-Y');
            ...
            $this->model_checkout_order->addOrderHistory($this->session->data['order_id'], $this->config->get('vindipix_order_status_id'), $comment, $notify = true);
            } else {
			...
			if ($order_info && $this->request->post['token_transaction'] && $this->request->post['transaction']['transaction_id'] && $order_info['payment_code'] == 'vindipix') {
				...
				switch($this->request->post['transaction']['status_id']) {
					case '4':
						...
						break;
					case '6':
						$order_status_id = $this->config->get('vindipix_order_status_id2');
						break;
					case '7':
						$order_status_id = $this->config->get('vindipix_order_status_id1');
						break;
					case '24':
						$order_status_id = $this->config->get('vindipix_order_status_id3');
						break;
					case '87':
						$order_status_id = $this->config->get('vindipix_order_status_id');
						break;
